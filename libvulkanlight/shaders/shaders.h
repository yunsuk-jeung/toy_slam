...
//pose color uv camera model
const std::string pose_color_uv_camera_model_vert = R"(
#version 450
#include "ShaderTypes.h"
layout(location = 0) in vec3 i_pos;
layout(location = 1) in vec3 i_col;
...
//color uv material sampler texture
const std::string color_uv_material_sampler_texture = R"(
#version 450
precision mediump float;
#include "ShaderTypes.h"
layout(location = 0) in vec3 i_col;
