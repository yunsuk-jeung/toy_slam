...
inline void read_resource_decoration<spv::DecorationLocation>(
  ...
  ShaderResource&              shader_resource,
  const ShaderVariant&         variant) {
  shader_resource.location = compiler.get_decoration(resource.id,
...
inline void read_resource_decoration<spv::DecorationDescriptorSet>(
  ...
  ShaderResource&              shader_resource,
  const ShaderVariant&         variant) {
  shader_resource.set = compiler.get_decoration(resource.id,
...
inline void read_resource_decoration<spv::DecorationInputAttachmentIndex>(
  ...
  const ShaderVariant&         variant) {
  shader_resource
    .input_attachment_index = compiler
                                .get_decoration(resource.id,
...
inline void read_shader_resource<ShaderResourceType::Input>(
  ...
  for (auto& resource : input_resources) {
    ...
    read_resource_decoration<spv::DecorationLocation>(compiler,
                                                      resource,
                                                      shader_resource,
                                                      variant);
...
inline void read_shader_resource<ShaderResourceType::InputAttachment>(
  ...
  for (auto& resource : subpass_resources) {
    ...
    read_resource_array_size(compiler, resource, shader_resource, variant);
    read_resource_decoration<spv::DecorationInputAttachmentIndex>(compiler,
                                                                  resource,
                                                                  shader_resource,
                                                                  variant);
    read_resource_decoration<spv::DecorationDescriptorSet>(compiler,
                                                           resource,
                                                           shader_resource,
                                                           variant);
    read_resource_decoration<spv::DecorationBinding>(compiler,
                                                     resource,
                                                     shader_resource,
                                                     variant);
...
inline void read_shader_resource<ShaderResourceType::ImageStorage>(
  ...
  for (auto& resource : storage_resources) {
    ...
    read_resource_array_size(compiler, resource, shader_resource, variant);
    read_resource_decoration<spv::DecorationNonReadable>(compiler,
                                                         resource,
                                                         shader_resource,
                                                         variant);
    read_resource_decoration<spv::DecorationNonWritable>(compiler,
                                                         resource,
                                                         shader_resource,
                                                         variant);
    read_resource_decoration<spv::DecorationDescriptorSet>(compiler,
...
void vkl::SPIRVReflection::reflectShaderRresources(vk::ShaderStageFlagBits      stage,
...
void SPIRVReflection::parseShaderResources(const spirv_cross::Compiler& compiler,
  ...
  read_shader_resource<ShaderResourceType::InputAttachment>(compiler,
                                                            stage,
                                                            resources,
                                                            variant);
  ...
  read_shader_resource<ShaderResourceType::ImageSampler>(compiler,
                                                         stage,
                                                         resources,
                                                         variant);
  read_shader_resource<ShaderResourceType::ImageStorage>(compiler,
                                                         stage,
                                                         resources,
                                                         variant);
  read_shader_resource<ShaderResourceType::Sampler>(compiler, stage, resources, variant);
  read_shader_resource<ShaderResourceType::BufferUniform>(compiler,
                                                          stage,
                                                          resources,
                                                          variant);
  read_shader_resource<ShaderResourceType::BufferStorage>(compiler,
                                                          stage,
                                                          resources,
                                                          variant);
}
...
void SPIRVReflection::parsePushConstants(const spirv_cross::Compiler& compiler,
  ...
  for (auto& resource : shader_resources.push_constant_buffers) {
    ...
    for (auto i = 0U; i < spivr_type.member_types.size(); ++i) {
      auto mem_offset = compiler.get_member_decoration(spivr_type.self,
                                                       i,
